#!/usr/bin/env python3
"""
MultiScaleCNN_Improved - Pipeline Orchestrator
====================================================

Orchestrates the complete MultiScaleCNN_Improved training and evaluation pipeline.
...
from data_loader import MultiScaleCNNDataLoader
from preprocessing import MultiScaleCNNPreprocessor
from model import MultiScaleCNNModel
from training import MultiScaleCNNTrainer
from evaluation import MultiScaleCNNEvaluator

class MultiScaleCNN_ImprovedPipeline:
    """Complete pipeline orchestrator for MultiScaleCNN_Improved model"""
    
    def __init__(self, output_dir="multiscalecnn_improved_results"):
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        # Initialize components
        self.data_loader = MultiScaleCNNDataLoader()
        self.preprocessor = MultiScaleCNNPreprocessor()
        self.model_builder = MultiScaleCNNModel()
        self.trainer = MultiScaleCNNTrainer(output_dir=output_dir)
        self.evaluator = MultiScaleCNNEvaluator(output_dir=output_dir)
        
        self.models = {}
        self.results = {}
        
        print(f"🎯 MultiScaleCNN_Improved Complete Pipeline Initialized")
        print(f"📁 Output directory: {self.output_dir}")
    
    def run_complete_pipeline(self, dataset_sizes=[250, 500, 750]):
        """Run complete pipeline for specified dataset sizes"""
        
        print(f"🚀 Running MultiScaleCNN_Improved Complete Pipeline")
        ...
        for dataset_size in dataset_sizes:
            ...
            try:
                ...
                model = self.model_builder.build_multiscalecnn_model(input_shape=(52, 2))
                
                # Step 6: Train model
                print("6️⃣ Training model...")
                training_results = self.trainer.compile_and_train_model(
                    model, X_train, y_train, X_val, y_val, dataset_size, "MultiScaleCNN_Improved"
                )
                
                # Step 7: Evaluate model
                print("7️⃣ Evaluating model...")
                evaluation_results = self.evaluator.evaluate_model(
                    model, X_test, y_test, "MultiScaleCNN_Improved", dataset_size
                )
                ...
                self.evaluator.plot_learning_curves(training_curves_data, "MultiScaleCNN_Improved", dataset_size)
                
                # Step 9: Save results
                print("9️⃣ Saving results...")
                self.evaluator.save_results("MultiScaleCNN_Improved", dataset_size, format='all')
        ...
        print(f"\n🎉 MultiScaleCNN_Improved Complete Pipeline Finished!")
