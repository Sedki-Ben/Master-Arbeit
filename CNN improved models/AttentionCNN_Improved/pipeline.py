#!/usr/bin/env python3
"""
AttentionCNN_Improved - Pipeline Orchestrator
===================================================

Orchestrates the complete AttentionCNN_Improved training and evaluation pipeline.
...
class AttentionCNN_ImprovedPipeline:
    """Complete pipeline orchestrator for AttentionCNN_Improved model"""
    ...
    def __init__(self, output_dir="attentioncnn_improved_results"):
        ...
        print(f"🎯 AttentionCNN_Improved Complete Pipeline Initialized")
        print(f"📁 Output directory: {self.output_dir}")
    
    def run_complete_pipeline(self, dataset_sizes=[250, 500, 750]):
        """Run complete pipeline for specified dataset sizes"""
        
        print(f"🚀 Running AttentionCNN_Improved Complete Pipeline")
        ...
        for dataset_size in dataset_sizes:
            ...
            try:
                ...
                training_results = self.trainer.compile_and_train_model(
                    model, X_train, y_train, X_val, y_val, dataset_size, "AttentionCNN_Improved"
                )
                
                # Step 7: Evaluate model
                print("7️⃣ Evaluating model...")
                evaluation_results = self.evaluator.evaluate_model(
                    model, X_test, y_test, "AttentionCNN_Improved", dataset_size
                )
                ...
                self.evaluator.plot_learning_curves(training_curves_data, "AttentionCNN_Improved", dataset_size)
                
                # Step 9: Save results
                print("9️⃣ Saving results...")
                self.evaluator.save_results("AttentionCNN_Improved", dataset_size, format='all')
        ...
        print(f"\n🎉 AttentionCNN_Improved Complete Pipeline Finished!")
