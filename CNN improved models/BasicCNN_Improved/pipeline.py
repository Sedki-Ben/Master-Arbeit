#!/usr/bin/env python3
"""
BasicCNN_Improved - Pipeline Orchestrator
===============================================

Orchestrates the complete BasicCNN_Improved training and evaluation pipeline.
...
class BasicCNN_ImprovedPipeline:
    """Complete pipeline orchestrator for BasicCNN_Improved model"""
    ...
    def run_complete_pipeline(self, dataset_sizes=[250, 500, 750]):
        ...
        for dataset_size in dataset_sizes:
            ...
            try:
                ...
                model = self.model_builder.build_basiccnn_model(input_shape=(52, 2))
                
                # Step 6: Train model
                print("6️⃣ Training model...")
                training_results = self.trainer.compile_and_train_model(
                    model, X_train, y_train, X_val, y_val, dataset_size, "BasicCNN_Improved"
                )
                
                # Step 7: Evaluate model
                print("7️⃣ Evaluating model...")
                evaluation_results = self.evaluator.evaluate_model(
                    model, X_test, y_test, "BasicCNN_Improved", dataset_size
                )
                ...
                self.evaluator.plot_learning_curves(training_curves_data, "BasicCNN_Improved", dataset_size)
                
                # Step 9: Save results
                print("9️⃣ Saving results...")
                self.evaluator.save_results("BasicCNN_Improved", dataset_size, format='all')
        ...
        print(f"\n🎉 BasicCNN_Improved Complete Pipeline Finished!")
