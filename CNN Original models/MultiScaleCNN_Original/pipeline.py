...
class MultiScaleCNN_OriginalPipeline:
    """Complete pipeline orchestrator for MultiScaleCNN_Original model"""
    
    def __init__(self, output_dir="multiscalecnn_original_results"):
        ...
        print(f"🎯 MultiScaleCNN_Original Complete Pipeline Initialized")
        print(f"📁 Output directory: {self.output_dir}")
    
    def run_complete_pipeline(self, dataset_sizes=[250, 500, 750]):
        """Run complete pipeline for specified dataset sizes"""
        
        print(f"🚀 Running MultiScaleCNN_Original Complete Pipeline")
        ...
        for dataset_size in dataset_sizes:
            ...
            try:
                ...
                training_results = self.trainer.compile_and_train_model(
                    model, X_train, y_train, X_val, y_val, dataset_size, "MultiScaleCNN_Original"
                )
                
                # Step 7: Evaluate model
                print("7️⃣ Evaluating model...")
                evaluation_results = self.evaluator.evaluate_model(
                    model, X_test, y_test, "MultiScaleCNN_Original", dataset_size
                )
                ...
                self.evaluator.plot_learning_curves(training_curves_data, "MultiScaleCNN_Original", dataset_size)
                
                # Step 9: Save results
                print("9️⃣ Saving results...")
                self.evaluator.save_results("MultiScaleCNN_Original", dataset_size, format='all')
        ...
        print(f"\n🎉 MultiScaleCNN_Original Complete Pipeline Finished!")
